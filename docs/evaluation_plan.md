...
| 检索质量 | 20 | Top-K relevance、duplicate rate、source success rate、source diversity、paper type diversity |
| 证据可信 | 25 | citation validity、claim evidence coverage、evidence matrix coverage、unsupported claim rate、hallucinated reference count |
...
| `source_diversity` | 候选来源多样性，用于衡量 mixed/hybrid 检索是否过度依赖单一数据库 |
| `paper_type_diversity` | 核心论文类型多样性，用于避免报告只由应用类或同质论文组成 |
| `evidence_matrix_coverage` | Evidence Matrix 覆盖核心论文比例 |
| `coverage_gap_count` | 覆盖检测后仍存在的结构性缺口数量 |
| `query_gap_recovery` | 是否通过补搜恢复覆盖缺口 |
