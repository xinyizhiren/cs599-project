...
当前实现支持离线样例数据和 arXiv API；DeepSeek 作为可选增强，用于证据抽取和报告背景段落润色。无 API Key 时系统仍可稳定运行。
...
- 参数：`--source offline|arxiv`、`--llm off|deepseek`、`--top-k`、`--output`。
...
| 报告质量 | 20 | section completeness、method taxonomy quality、research gap usefulness |
...
$env:DEEPSEEK_API_KEY="your_deepseek_api_key"
$env:PYTHONPATH="src"
python -m researchflow run "Agentic RAG for enterprise knowledge management" --source arxiv --llm deepseek --top-k 5
```
...
- 接入 Semantic Scholar，补充引用数、相关论文和作者信息。
