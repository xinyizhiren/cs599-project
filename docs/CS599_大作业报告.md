...
本项目的差异化主要体现在四点。第一，检索召回从单源 arXiv 扩展为 Mixed Research Retrieval，`hybrid` 覆盖 arXiv、Semantic Scholar、Crossref、OpenAlex，`mixed` 进一步支持可选 Tavily Web 背景源。第二，查询规划从平铺 query list 升级为 research questions、subtopics、query tree，并通过 coverage gap detector 自动补搜缺失方向。第三，大规模信息理解不依赖一次性把所有材料塞进 LLM，而是先做候选池画像和论文类型平衡，再生成 Evidence Matrix 与 Claim Graph，使报告按研究问题、方法谱系和证据关系组织。第四，最终产物从“论文列表式摘要”升级为完整中文调研报告，包含检索策略、时间分布、方法分类、核心论文精读、方法对比、评测指标、主要结论与证据、争议局限、未来方向和附录。
