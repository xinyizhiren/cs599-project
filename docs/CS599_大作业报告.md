...
系统最终产物包括三类文件：一是面向快速决策的最终综合总结，二是面向展开阅读的完整文献调研报告，三是面向复现和审计的调研过程记录。最终综合总结把论文集合压缩成一句话结论、关键发现、技术脉络、主要难点和工程建议；过程记录则保留检索式、联网数据源、候选论文、Top-K 筛选、证据台账、Claim-Evidence 对齐、引用校验和评估指标，但不输出大模型隐藏推理链。
